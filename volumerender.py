...
def rayTransfer(voxels):
    ...
    for x in voxels:
        ...
        r = 1 # <- composite new rgb values 
        g = 1 # <- composite new rgb values 
        b = 1 # <- composite new rgb values 
...
def maxIntensity(voxels):
    maximum = 0
    # TODO: Find the max intensity and use it to calculate rgb
    # HINT: Use rn, gn, bn equations from rayTransfer to calculate final rgb values

    return r,g,b,a

def avg(voxels):
    r,g,b,a = [0,0,0,0]
    # TODO: Find the average intensity and use it to calculate rgb
